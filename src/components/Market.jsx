...
export default function Market({ 
  ...
  const packs = [
    { id: 'basic', name: 'BASIC DATACACHE', cost: 500, icon: '💾', num: 3, desc: 'Enthält 3 Karten. Standard-Dropraten. Gut für den Start.' },
    { id: 'premium', name: 'EXECUTIVE ARCHIVE', cost: 2500, icon: '🗄️', num: 5, desc: 'Enthält 5 Karten. Erhöhte Wahrscheinlichkeit auf Apex & Legacy.' },
    { id: 'apex', name: 'BLACK MARKET OVERRIDE', cost: 8000, icon: '👑', num: 1, desc: 'Zieht nur 1 Karte! Massive Chance auf Apex/Legacy.' }
  ];
  ...
  const getRandomCard = (packType) => {
    ...
    // ULTIMATIVER JSON-BYPASS: Holt ausnahmslos ALLE Objekte aus der JSON.
    // Egal ob das Array "characters", "effects", "taktiken" oder sonst wie heißt.
    let allCards = [];
    if (Array.isArray(cardsData)) {
      allCards = [...cardsData];
    } else {
      allCards = Object.values(cardsData).flat().filter(item => typeof item === 'object' && item !== null);
    }
    
    let pool = allCards.filter(c => {
      // Effektkarte erkennen (egal ob über 'type' oder Buff-Eigenschaften)
      if (selectedCardType === 'effect') {
        return c.type === 'effect' || c.buff !== undefined || c.buffPercent !== undefined || c.cost !== undefined;
      }
      // Standardkarte erkennen
      if (selectedCardType === 'std') {
        return c.type === 'std' || (!c.type && c.buff === undefined && c.cost === undefined);
      }
      return c.type === selectedCardType;
    });

    // Logging & Fallback, falls der Pool leer ist
    if (pool.length === 0) {
      console.warn(`[MARKET ERROR] Pool leer für Typ: ${selectedCardType}. Prüfe die cards.json! Fallback auf Agenten.`);
      pool = allCards.filter(c => c.type === 'std' || (!c.type && c.buff === undefined));
    }
