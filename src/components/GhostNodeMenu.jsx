...
export default function GhostNodeMenu({ session, baseHp, setBaseHp, avatarCard, roguelikeRun, allRuns = {}, onGoToLab, onGoToSquad, onGoToMap, onBack, friends = [], onInviteDirect, onDeleteCoop }) {
  ...
  // Lokaler State für flüssiges Sliden (verhindert Datenbank-Crash)
  const [localBaseHp, setLocalBaseHp] = React.useState(baseHp);
  ...
  // Synchronisiert lokalen Slider mit eingehenden Cloud-Updates
  React.useEffect(() => {
    setLocalBaseHp(baseHp);
  }, [baseHp]);
  ...
  const isAdmin = username === 'MANU' || username === 'LEON' || username === 'ELSON'; // Added Elson per prompt instruction
      ...
      return (
        ...
        <div className="gn-main-layout" style={{
          /* MOBILE OPTIMIZATION START */
          ...
          <div style={{
            display:'flex',flexDirection:'column',alignItems:'center',gap:'15px',flexShrink:0,
            /* MOBILE OPTIMIZATION START */
